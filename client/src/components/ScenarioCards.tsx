      ...
      {scenarios.map((s, i) => {
        ...
        return (
          <div
            key={s.label}
            className={cn(
              "bg-card border border-card-border rounded-lg p-4 border-l-[3px] flex flex-col",
              colors[i]
            )}
          ...
          >
            ...
            <div className="space-y-1.5 mb-3">
              ...
              <div className="flex justify-between items-baseline">
                ...
                <span className="tabular-nums text-xs font-semibold">{fmtCAD(s.principal + (selectedTerm === 30 ? s.cmhcPremium30 : s.cmhcPremium25), true)}</span>
              </div>
              {s.isCMHC && (selectedTerm === 30 ? s.cmhcPremium30 : s.cmhcPremium25) > 0 && (
                <div className="flex justify-between items-baseline">
                  <span className="text-[11px] text-muted-foreground" title="Provincial sales tax may apply">CMHC premium</span>
                  <span className="tabular-nums text-[11px] text-amber-600 dark:text-amber-400">+{fmtCAD(selectedTerm === 30 ? s.cmhcPremium30 : s.cmhcPremium25, true)}</span>
                </div>
              )}
            </div>

            {/* Divider */}
            <div className="border-t border-border mt-auto mb-2.5 pt-2" />
